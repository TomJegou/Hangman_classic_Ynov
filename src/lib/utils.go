...
*/
func ChoseRandomWord(wordDictionnary []string) string {
	return wordDictionnary[rand.New(rand.NewSource(time.Now().UnixNano())).Intn(len(wordDictionnary))] // index number generated by the function rand using time as a seed and ranged between 0 and dic's length
}

/*
Fuction used to change the "_" to the corresponding letter from the word to guess
*/
func DiscoverLetter(currentStateWord []byte, letter string, word string) {
	for i := 0; i < len(word); i++ {
		if string(word[i]) == letter {
			currentStateWord[i] = word[i]
		}
	}
}
...
*/
func IsIn(table []string, char string) bool {
	for i := 0; i < len(table); i++ {
		if string(table[i]) == char {
			return true
		}
	}
	return false
...
*/
func ListMap(mapDirName map[string]string) []string {
	keys := []string{}
	for key := range mapDirName {
		keys = append(keys, key)
	}
	sort.Strings(keys)
	for index_key, key := range keys {
		PrintColor("["+key+"]"+": "+mapDirName[keys[index_key]]+"\n", "White")
	}
	return keys
}
...
*/
func RemainingLetter(currentStateWord []byte, completeWord string) []string {
	result := []string{}
	for i := 0; i < len(currentStateWord); i++ {
		if !IsIn(result, string(currentStateWord[i])) && string(currentStateWord[i]) == "_" && !IsIn(result, string(completeWord[i])) {
